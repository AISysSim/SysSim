...
def sdpa_flop_count(query_shape, key_shape, value_shape):
    """
    Count flops for self-attention, including GQA (Grouped Query Attention).

    For GQA, key/value head count (nkv) may differ from query head count (nh).
    FLOPs are computed based on nh query heads since each query head performs
    a full attention pass (kv heads are broadcast/repeated across query groups).
    """
    b, h, s_q, d_q = query_shape
    _b2, _h2, s_k, _d2 = key_shape
    _b3, _h3, _s3, d_v = value_shape
    if not b == _b2 == _b3 or not d_q == _d2 or not s_k == _s3:
    ...
    *,
    ...
    if cu_seqlens_q is not None:
        ...
        if cu_seqlens_q.shape != cu_seqlens_k.shape:
            raise AssertionError(
                "_unpack_efficient_attention_nested_shapes: cu_seqlens_q and cu_seqlens_k must have the same shape"
            )
