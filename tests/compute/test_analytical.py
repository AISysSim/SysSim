...
def test_large_gemm_binds_on_tensor_not_memory():
    ...
    b = analytical_bound(fp, args, kw, out, HW, OperatorType.GEMM)
    ...
    assert b.roofline_hw_ns == max(b.tensor_ns, b.fma_ns, b.sfu_ns, b.mem_ns)
    assert b.t_an_ns == b.tensor_ns  # large GEMM is compute-bound


def test_launch_floor_dominates_tiny_op():
    fp, args, kw, out = _mm(1, 1, 1)
    b = analytical_bound(fp, args, kw, out, HW, OperatorType.GEMM, t_launch_ns=5000.0)
    assert b.t_an_ns == 5000.0           # launch floor wins for a tiny op
    assert b.roofline_hw_ns < 5000.0     # hw bound (no launch) is smaller
