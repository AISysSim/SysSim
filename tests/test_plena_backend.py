...
# Mark all tests in this module as 'plena' so CI can exclude with -m "not plena"
pytestmark = pytest.mark.plena
...
class TestCycleEstimation:
    ...
    def test_gemm_cycles_positive(self, perf_model):
        ...
        cycles = estimate_plena_cycles_for_gemm(perf_model, batch=1, m=128, n=256, k=512)
        assert cycles > 0
    ...
    def test_gemm_cycles_scale_with_size(self, perf_model):
        ...
        cycles_small = estimate_plena_cycles_for_gemm(perf_model, batch=1, m=64, n=64, k=64)
        cycles_large = estimate_plena_cycles_for_gemm(perf_model, batch=1, m=512, n=512, k=512)
